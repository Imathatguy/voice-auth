...
for n, (ax, clasif) in enumerate(zip(axes, arch_order)):
    ...
    # ax.set_title(clasif)
    if not verify_data:
        extra_str = " {} reppetitions".format(len(data_holder[clasif][0.00]))
    else:
        extra_str = ""
    ax.set_xlabel('{} threshold{}'.format(str.upper(clasif), extra_str))
    ax.set_ylim(0, 1)
    ax.set_xlim(0, 1)
    if n == 0:
        ax.set_ylabel('Error')
    ax.legend()
    # plt.show()
plt.savefig('voice_betadefence_roc.pdf', bbox_inches='tight')
plt.show()
