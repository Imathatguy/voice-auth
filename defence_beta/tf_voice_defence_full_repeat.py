...
from defend_voice import generate_protection_noise
...
def main_run(selection, n_users, ratio):
    resample_users = True

    # Create a population statistic for the data
    if resample_users:
        embeddings_loc = "./../voice_embeddings.csv"

        n_feat = 1024

        a = VoiceUserPopulation(embeddings_loc, n_feat, n_users)
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        ...
        print(selection, n_users, ratio)
        results_holder.append(main_run(selection, n_users, ratio))
