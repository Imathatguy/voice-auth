function [embed, net] = vggvox_embeddings(audio_path, varargin)
...
inp2 = test_getinput(inpPath1, net.meta, buckets);
...
% net.eval({ 'input_b1', gpuArray(inp1)}, );

net.eval({ 'input_b1', gpuArray(inp1) ,'input_b2', gpuArray(inp2) });

% dist = gather(squeeze(net.vars(featid).value));
% b = gather(squeeze(net.vars(54).value));
i = net.getVarIndex('x1_s1');
embed = gather(squeeze(net.vars(i).value));
% b
% Print distance
